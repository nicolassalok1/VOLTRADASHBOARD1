...
def _migrate_legacy_spy_cache() -> None:
    ...
    """
    try:
        _CACHE_SPY_DIR.mkdir(parents=True, exist_ok=True)
        for legacy_path in (_LEGACY_CACHE_SPY_CLOSE, _LEGACY_CACHE_SPY_CLOSE_OLD):
            if legacy_path.exists():
                legacy_path.replace(_CACHE_SPY_CLOSE)
    except Exception:
        pass


def _save_closing_cache(series: pd.Series, cache_path: Path | None = None) -> None:
    """Persist close series and reset age indicator."""
    global _CLOSING_CACHE_AGE_HOURS
    try:
        target = cache_path or _CACHE_SPY_CLOSE
        target.parent.mkdir(parents=True, exist_ok=True)
        series.to_csv(target, index_label="date")
        _CLOSING_CACHE_AGE_HOURS = 0.0
    except Exception:
        pass
...
def fetch_spy_history(period: str = "1y", interval: str = "1d", cache_path: Path | None = None, ticker: str = "SPY") -> pd.Series:
    """Fetch close prices for the given ticker with a single CSV cache (overwrites per ticker)."""
    _migrate_legacy_spy_cache()
    ticker_norm = (ticker or "SPY").strip().upper()
    effective_cache = cache_path or _CACHE_SPY_CLOSE
    try:
        if effective_cache.exists():
            cached = pd.read_csv(effective_cache, index_col=0, parse_dates=True)
...
def last_spy_close(period: str = "1y", interval: str = "1d", ticker: str = "SPY") -> float:
    _migrate_legacy_spy_cache()
    close = fetch_spy_history(period=period, interval=interval, ticker=ticker)
